...
require_once __DIR__ . '/../../../module/Users/src/Users/Entity/Role.php';
require_once __DIR__ . '/../../../module/Utilities/src/Utilities/Service/Status.php';
 ...
 */
{
    ...
    return array(
        'login' => $relative_url_prefix . '../sign/in',
        'register' => $relative_url_prefix . '../users/new',
        'logout' => $relative_url_prefix . '../sign/out',
    );
 ...
 */
{
    ...
    if ($auth->hasIdentity()) {
        ...
        if (in_array(Role::ADMIN_ROLE, $storage['roles'])) {
            $loggedinUserData["level"] = QA_USER_LEVEL_ADMIN;
        }
        else {
            $loggedinUserData["level"] = QA_USER_LEVEL_BASIC;
        }
        $loggedinUserData["userid"] = $storage['id'];
        $loggedinUserData["publicusername"] = $storage['username'];
        $loggedinUserData["email"] = $storage['email'];
        if ($storage['status'] !== Status::STATUS_ACTIVE) {
            $loggedinUserData["blocked"] = true;
        }
    }
 ...
 */
{
    ...
    $qa_db_connection = qa_db_pdo_connection();

    $statement = $qa_db_connection->prepare("SELECT email FROM user WHERE id=?");
 ...
 */
{
    ...
    if (count($publicusernames)) {
        ...
        $publicUsernamesString  = str_pad('',count($publicusernames)*2-1,'?,');;
        $statement = $qa_db_connection->prepare("SELECT username, id FROM user WHERE username IN ( $publicUsernamesString )");
        $statement->execute($publicusernames);
 ...
 */
{
    ...
    if (count($userids)) {
        $qa_db_connection = qa_db_pdo_connection();
        $publicIdsString  = str_pad('',count($userids)*2-1,'?,');
        $statement = $qa_db_connection->prepare("SELECT username, id FROM user WHERE id IN ( $publicIdsString )");
...
function qa_db_pdo_connection($connect = true)
{
    ...
    if ($connect && is_null($qa_db_pdo_connection)) {
        ...
        $qa_db_pdo_connection = new PDO("mysql:host=$host;dbname=$dbname$port", $username, $password, array(
